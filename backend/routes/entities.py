...
import database
import logging # On ajoute le module de logging pour mieux voir les erreurs
...
# --- MODIFICATION DE LA GESTION D'ERREUR DANS LA ROUTE DE VALIDATION ---
...
def validate_entity(entity_type, entity_id):
    ...
    db_conn = None
    cursor = None # On déclare le curseur en dehors du try
    try:
        db_conn = database.get_connection()
        if not db_conn:
        ...
        # Étape 2: Mettre à jour (on peut réutiliser le même curseur)
        ...
        # On log l'erreur complète dans le terminal pour le débogage
        ...
        # On s'assure de fermer le curseur et la connexion
        if cursor:
            cursor.close()
        if db_conn and db_conn.is_connected():
            db_conn.close()