...
def main_orchestrator():
    ...
    try:
        ...
        # --- DÉBUT DE LA MODIFICATION DE LOGIQUE ---
        # On ne récupère que les fichiers qui ont un statut de succès pour les ignorer.
        # Ceux en erreur seront retraités.
        fichiers_succes = database.get_fichiers_traites(db_conn) 
        tous_les_fichiers_drive = google_drive.lister_fichiers_recursif(drive_service, GOOGLE_DRIVE_FOLDER_ID)
        fichiers_a_traiter = [f for f in tous_les_fichiers_drive if f['id'] not in fichiers_succes]
        ...
        for fichier in fichiers_a_traiter:
            ...
            try:
                ...
                # On change le statut si aucun manuel n'est trouvé
                if total_manuels == 0:
                    log_status, log_message = 'ERREUR_EXTRACTION', "Aucun manuel n'a été extrait par l'IA."
                    print(f"  -> ERREUR: {log_message}")
                    if not TEST_MODE: database.log_to_db(db_conn, fichier['id'], fichier['name'], log_status, log_message)
                    continue
